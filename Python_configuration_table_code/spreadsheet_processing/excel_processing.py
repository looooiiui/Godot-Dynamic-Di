...
class ExcelManager:
    ...
    def __init__(self, file_path: Optional[str] = None):
        ...
        if self._inner_excel_path is not None:
            try:
                read_excel: Workbook = load_workbook(file_path, 
                                                     read_only=True
                                                     )
    ...
    def close_workbook(self) -> bool:
        ...
        try:
            ...
            DebugTool.debug_log(f"Excel统一类: 表单关闭成功")
            return True
...
def excel_sheet_get_pure_max_row(input_sheet: Optional[Worksheet], col_index: int = 1) -> int:
    ...
    try:
        ...
        cell_value = input_sheet.cell(row = 1, column = 1).value
        if cell_value is None or str(cell_value).strip() == "":
            DebugTool.debug_log(f"表格第一行第一个为空")
            return -1
